# Copyright 2019-202 ETH Zurich and the DaCe authors. All rights reserved.
""" Contains inter-state transformations of an SDFG to run on the Tenstorrent Wormhole. """
...
wormhole_storage = [dtypes.StorageType.Wormhole_DRAM]


def _recursive_out_check(node, state, wormhole_scalars):
    """
    Recursively checks if the outputs of a node are scalars and if they are/should be stored in Wormhole memory.
    """
    scalset = set()
    scalout = True
    sdfg = state.parent
    for e in state.out_edges(node):
        ...
        if isinstance(last_edge.dst, nodes.AccessNode):
            ...
            if isinstance(desc, data.Scalar):
                if desc.storage in wormhole_storage or last_edge.dst.data in wormhole_scalars:
                    scalout = False
                scalset.add(last_edge.dst.data)
                sset, ssout = _recursive_out_check(last_edge.dst, state, wormhole_scalars)
            ...
            if desc.shape == (1, ):  # Pseudo-scalar
                ...
                scalout = scalout and ssout
                continue
            if desc.storage not in wormhole_storage and last_edge.data.num_elements() == 1:
                sset, ssout = _recursive_out_check(last_edge.dst, state, wormhole_scalars)
                scalset = scalset.union(sset)
                scalout = scalout and ssout
                continue
...
def _recursive_in_check(node, state, wormhole_scalars):
    """
    Recursively checks if the inputs of a node are scalars and if they are/should be stored in Wormhole memory.
    """
    scalset = set()
    scalout = True
    sdfg = state.parent
    for e in state.in_edges(node):
        ...
        if isinstance(last_edge.src, nodes.AccessNode):
            ...
            if isinstance(desc, data.Scalar):
                if desc.storage in wormhole_storage or last_edge.src.data in wormhole_scalars:
                    scalout = False
                scalset.add(last_edge.src.data)
                sset, ssout = _recursive_in_check(last_edge.src, state, wormhole_scalars)
                scalset = scalset.union(sset)
                scalout = scalout and ssout
                continue
            if desc.shape == (1, ):  # Pseudo-scalar
...
class WormholeTransformSDFG(transformation.MultiStateTransformation):
    """ Implements the WormholeTransformSDFG transformation.

        Transforms a whole SDFG to run on the Tenstorrent Wormhole:

            1. Acquire metadata about SDFG and arrays
            2. Replace all non-transients with their Wormhole counterparts
            3. Copy-in state from host to Wormhole
            4. Copy-out state from Wormhole to host
            5. Re-store Default-top/CPU_Heap transients as Wormhole_DRAM
            6. Global tasklets are wrapped with a map of size 1
            7. Global Maps are re-scheduled to use the Wormhole
            8. Make data ready for interstate edges that use them
            9. Re-apply simplification to get rid of extra states and transients
    """

    toplevel_trans = Property(desc="Make all Wormhole transients top-level", dtype=bool, default=True)

    register_trans = Property(desc="Make all transients inside Wormhole maps registers", dtype=bool, default=True)
    ...
    host_maps = ListProperty(desc='List of map GUIDs, the passed maps are not offloaded to the Wormhole',
                             element_type=str,
                             default=None,
                             allow_none=True)

    host_data = ListProperty(desc='List of data names, the passed data are not offloaded to the Wormhole',
                             element_type=str,
                             default=None,
                             allow_none=True)
    ...
    def apply(self, _, sdfg: sd.SDFG):
        ...
        #######################################################
        # Step 1: Create cloned Wormhole arrays and replace originals

        data_already_on_wormhole = {}

        cloned_arrays = {}
        for inodename, inode in set(input_nodes):
            if inode.storage == dtypes.StorageType.Wormhole_DRAM:
                data_already_on_wormhole[inodename] = None
                continue
            if isinstance(inode, data.Scalar):  # Scalars can remain on host
                continue
            newdesc = inode.clone()
            newdesc.storage = dtypes.StorageType.Wormhole_DRAM
            newdesc.transient = True
            name = sdfg.add_datadesc('wormhole_' + inodename, newdesc, find_new_name=True)
            cloned_arrays[inodename] = name

        for onodename, onode in set(output_nodes):
            if onode.storage == dtypes.StorageType.Wormhole_DRAM:
                data_already_on_wormhole[onodename] = None
                continue
            ...
            newdesc.storage = dtypes.StorageType.Wormhole_DRAM
            newdesc.transient = True
            name = sdfg.add_datadesc('wormhole_' + onodename, newdesc, find_new_name=True)
            ...
            # the entire array to the Wormhole.
        ...
        for mem in check_memlets:
            if sdfg.arrays[mem.data].storage == dtypes.StorageType.Wormhole_DRAM:
                data_already_on_wormhole[mem.data] = None
        ...
        #######################################################
        # Step 4: Change all top-level maps and library nodes to Wormhole schedule

        wormhole_nodes: Set[Tuple[SDFGState, nodes.Node]] = set()
        for state in sdfg.states():
            sdict = state.scope_dict()
            for node in state.nodes():
                if sdict[node] is None:
                    if isinstance(node, (nodes.LibraryNode, nodes.NestedSDFG)):
                        if node.guid:
                            node.schedule = dtypes.ScheduleType.Wormhole_Kernel
                            wormhole_nodes.add((state, node))
        ...
        # NOTE: The outputs of LibraryNodes, NestedSDFGs and Map that have Wormhole schedule must be moved to Wormhole memory.
        for state, node in wormhole_nodes:
            if isinstance(node, (nodes.LibraryNode, nodes.NestedSDFG)):
                for e in state.out_edges(node):
                    dst = state.memlet_path(e)[-1].dst
                    if isinstance(dst, nodes.AccessNode):
                        desc = sdfg.arrays[dst.data]
                        desc.storage = dtypes.StorageType.Wormhole_DRAM
        ...
        #######################################################
        # Step 5: Collect free tasklets and check for scalars that have to be moved to the Wormhole
        # Also recursively call WormholeTransformSDFG on NestedSDFGs that have Wormhole device schedule but are not actually
        # inside a Wormhole kernel.

        wormhole_scalars = {}
        nsdfgs: List[Tuple[nodes.NestedSDFG, SDFGState]] = []
        changed = True
        # Iterates over Tasklets that not inside a Wormhole kernel. Such Tasklets must be moved inside a Wormhole kernel only
        # if they write to Wormhole memory. The check takes into account the fact that Wormhole kernels can read host-based
                ...
                for node in state.nodes():
                    ...
                    if isinstance(node, nodes.NestedSDFG):
                        ...
                        if node in global_code_nodes[state]:
                            continue
                        if state.entry_node(node) is None: # and not scope.is_devicelevel_gpu_kernel(state.parent, state, node):
                            scalars, scalar_output = _recursive_out_check(node, state, wormhole_scalars)
                            sset, ssout = _recursive_in_check(node, state, wormhole_scalars)
                            scalars = scalars.union(sset)
                            scalar_output = scalar_output and ssout
                            csdfg = state.parent
                            # If the tasklet is not adjacent only to scalars or it is in a Wormhole scope.
                            # The latter includes NestedSDFGs that have a Wormhole-Device schedule but are not in a Wormhole kernel.
                            if (not scalar_output
                                    or (csdfg.parent is not None
                                        and csdfg.parent_nsdfg_node.schedule == dtypes.ScheduleType.Wormhole_Kernel)):
                                global_code_nodes[state].append(node)
                                wormhole_scalars.update({k: None for k in scalars})
                                changed = True

        # Apply WormholeTransformSDFG recursively to NestedSDFGs.
        for node, state in nsdfgs:
            excl_copyin = set()
            for e in state.in_edges(node):
                src = state.memlet_path(e)[0].src
                if isinstance(src, nodes.AccessNode) and sdfg.arrays[src.data].storage in wormhole_storage:
                    excl_copyin.add(e.dst_conn)
                    node.sdfg.arrays[e.dst_conn].storage = sdfg.arrays[src.data].storage
            excl_copyout = set()
            for e in state.out_edges(node):
                dst = state.memlet_path(e)[-1].dst
                if isinstance(dst, nodes.AccessNode) and sdfg.arrays[dst.data].storage in wormhole_storage:
        ...
        for state in sdfg.states():
            ...
            for node in state.nodes():
                if isinstance(node, nodes.AccessNode) and node.desc(sdfg).transient:
                    ...
                    if sdict[node] is None and nodedesc.storage not in wormhole_storage:

                        # Scalars were already checked.
                        if isinstance(nodedesc, data.Scalar) and not node.data in wormhole_scalars:
                            continue

                        # NOTE: the cloned arrays match too but it's the same storage so we don't care
                        if node.data not in self.host_data:
                            nodedesc.storage = dtypes.StorageType.Wormhole_DRAM
                    ...
                    elif nodedesc.storage not in wormhole_storage:
                        # Make internal transients registers
                        if self.register_trans:
                            nodedesc.storage = dtypes.StorageType.Register

        #######################################################
        # Step 7: Wrap free tasklets and nested SDFGs with a Wormhole map

        # Extend global_code_nodes with tasklets that write/read from an array
        # Previous steps map all arrays to Wormhole storage, but only checks tasklets that write to/read from
        # Scalars to be wrapped in a Wormhole Map
        for state in sdfg.states():
            for node in state.nodes():
                if isinstance(node, nodes.Tasklet):
                    ...
                    if state.entry_node(node) is None: # and not scope.is_devicelevel_gpu_kernel(state.parent, state, node):
                        ...
                        wormhole_accesses = [
                            ...
                            if isinstance(n, nodes.AccessNode) and sdfg.arrays[n.data].storage in wormhole_storage
                        ]
                        if len(wormhole_accesses) > 0:
                            global_code_nodes[state].append(node)
        ...
        for state, gcodes in global_code_nodes.items():
            for gcode in gcodes:
                ...
                me, mx = state.add_map(gcode.label + '_gmap', {gcode.label + '__gmapi': '0:1'},
                                       schedule=dtypes.ScheduleType.Wormhole_Kernel)
        ...
        cloned_data = set(cloned_arrays.keys()).union(wormhole_scalars.keys()).union(data_already_on_wormhole.keys())

        def _create_copy_out(arrays_used: Set[str]) -> Dict[str, str]:
            # Add copy-out nodes
            name_mapping = {}
            for nname in arrays_used:
                # Handle Wormhole scalars
                if nname in wormhole_scalars:
                    hostname = wormhole_scalars[nname]
                    if not hostname:
                        ...
                        wormhole_scalars[nname] = hostname
                    else:
                        desc = sdfg.arrays[hostname]
                    devicename = nname
                elif nname in data_already_on_wormhole:
                    hostname = data_already_on_wormhole[nname]
                    if not hostname:
                        desc = sdfg.arrays[nname].clone()
                        desc.storage = dtypes.StorageType.CPU_Heap
                        desc.transient = True
                        hostname = sdfg.add_datadesc('host_' + nname, desc, find_new_name=True)
                        data_already_on_wormhole[nname] = hostname
                    else:
                        desc = sdfg.arrays[hostname]
                    devicename = nname
                else:
